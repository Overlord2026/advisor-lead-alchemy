...
const FeatureFlagToggler = () => {
  ...
  if (!isVisible) {
    return (
      <div 
        className="fixed bottom-4 right-4 p-2 bg-primary text-white rounded-full shadow-lg cursor-pointer z-50"
        onClick={() => setIsVisible(true)}
        data-testid="feature-flag-toggle-button"
      >
        FF
      </div>
    );
  ...
  return (
    <Card className="fixed bottom-4 right-4 p-4 shadow-xl w-80 max-h-[80vh] overflow-auto z-50" data-testid="feature-flag-panel">
      ...
      <div className="space-y-3">
        {featureEntries.map(([feature, enabled]) => (
          <div 
            ...
            data-testid={`feature-flag-${feature}`}
          >
            <div>
