...
export default function StartRecordingDialog({ open, onOpenChange, onStartRecording }: StartRecordingDialogProps) {
  ...
  const handleSaveRecording = (blob: Blob, duration: number) => {
    // In a real app, you would upload the blob to storage
    console.log(`Saving recording: ${title}, duration: ${duration}s, size: ${Math.round(blob.size / 1024)} KB`);
